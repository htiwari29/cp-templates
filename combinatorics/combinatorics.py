...
def nCr(n, r): return reduce(op.mul, range(
    n - r + 1, n + 1), 1) // math.factorial(r)


def multinomial(k): return math.factorial(
    sum(k)) // reduce(op.mul, (math.factorial(i) for i in k))


def derangements(n): return int(math.factorial(n) / math.e + 0.5)


def bell(n): return sum(stirling_2_recursive(k, n) for k in range(n + 1))


def catalan(n): return nCr(2 * n, n) // (n + 1)


def euler(n, k): return sum((1 - 2 * (j & 1)) * nCr(n + 1, j)
                            * ((k + 1 - j)**n) for j in range(k + 1))


def stirling_2(n, k): return sum(((-1)**(k - j)) * nCr(k, j) * (j**n)
